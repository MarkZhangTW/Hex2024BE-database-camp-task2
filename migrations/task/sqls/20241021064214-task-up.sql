...
SELECT
    ...
    p.price
FROM "USER" u
INNER JOIN "CREDIT_PACKAGE" p
ON u."name" = '王小明' AND p."name" = '14 堂組合包方案';

INSERT INTO "CREDIT_PURCHASE" (user_id, credit_package_id, purchased_credits, price_paid)
SELECT u.id, cp.id, cp.credit_amount, cp.price FROM "USER" u JOIN "CREDIT_PACKAGE" cp ON u."name" = '王小明' AND cp."name" = '21 堂組合包方案';

INSERT INTO "CREDIT_PURCHASE" (user_id, credit_package_id, purchased_credits, price_paid)
SELECT u.id, cp.id, cp.credit_amount, cp.price FROM "USER" u JOIN "CREDIT_PACKAGE" cp ON u."name" = '好野人' AND cp."name" = '14 堂組合包方案';
...
SELECT
    ...
    '即將授課'
FROM "USER" u
INNER JOIN "USER" coach ON coach."role" = 'COACH' AND coach.email = 'lee2000@hexschooltest.io'
INNER JOIN "COURSE" course ON course.user_id = coach.id
...
UPDATE "COURSE_BOOKING" cb
SET
    cancelled_at = '2024-11-24 17:00:00',
    status = '課程已取消'
FROM "USER" u
WHERE cb.user_id = u.id AND u.email = 'wXlTq@hexschooltest.io';
...
SELECT
    u.name          AS "預約會員",
    c."name"        AS "預約課程",
    cb.booking_at   AS "預約時間",
    cb.status       AS "預約狀態",
    cb.cancelled_at AS "取消預約時間"
FROM "USER" u
INNER JOIN "COURSE_BOOKING" cb ON cb.user_id = u.id
INNER JOIN "COURSE" c ON c.id  = cb.course_id
WHERE u.email = 'wXlTq@hexschooltest.io';
...
SET
    ...
    status = '上課中'
FROM "USER" u
WHERE cb.user_id = u.id AND cancelled_at IS NULL AND u.email = 'wXlTq@hexschooltest.io';

-- 5-6. 查詢：計算用戶王小明的購買堂數，顯示須包含以下欄位： user_id , total。 (需使用到 SUM 函式與 Group By)
SELECT u.id AS user_id, u.name, SUM(cp.purchased_credits) AS total
FROM "USER" u
INNER JOIN "CREDIT_PURCHASE" cp ON cp.user_id = u.id
WHERE u.email = 'wXlTq@hexschooltest.io'
GROUP BY u.id;

-- 5-7. 查詢：計算用戶王小明的已使用堂數，顯示須包含以下欄位： user_id , total。 (需使用到 Count 函式與 Group By)
SELECT u.id AS user_id, u.name, COUNT(cb.id) AS total FROM "USER" u
INNER JOIN "COURSE_BOOKING" cb ON cb.user_id = u.id AND join_at IS NOT NULL
WHERE u.email = 'wXlTq@hexschooltest.io'
GROUP BY u.id;
...
SELECT u.id AS user_id, u.name, cp.total_credit - cb.used_credit AS remaining_credit
FROM "USER" u
INNER JOIN (
...
LEFT JOIN (
    SELECT user_id, COUNT(id) AS used_credit
    FROM "COURSE_BOOKING" WHERE join_at IS NOT NULL
    GROUP BY user_id
) AS cb ON cb.user_id = u.id
WHERE u.email = 'wXlTq@hexschooltest.io';
...
INNER JOIN "COACH_LINK_SKILL" cls ON cls.coach_id = c.id
INNER JOIN "SKILL" s ON s.id = cls.skill_id AND s."name" = '重訓'
...
SELECT
    package."name"      AS "組合包方案名稱",
    COUNT(purchase.id)  AS "銷售數量"
FROM "CREDIT_PURCHASE" purchase
INNER JOIN "CREDIT_PACKAGE" package ON package.id = purchase.credit_package_id
WHERE purchase.purchase_at BETWEEN '2024-11-01' AND '2024-11-30'
GROUP BY package.id;

-- 6-4. 查詢：計算 11 月份總營收（使用 purchase_at 欄位統計）
-- 顯示須包含以下欄位： 總營收
SELECT SUM(price_paid) AS "總營收" FROM "CREDIT_PURCHASE" cp
WHERE purchase_at BETWEEN '2024-11-01' AND '2024-11-30';

-- 6-5. 查詢：計算 11 月份有預約課程的會員人數（需使用 Distinct，並用 created_at 和 status 欄位統計）
-- 顯示須包含以下欄位： 預約會員人數
SELECT COUNT(DISTINCT user_id) FROM "COURSE_BOOKING" cb
WHERE status != '課程已取消' AND created_at BETWEEN '2024-11-01' AND '2024-11-30';
